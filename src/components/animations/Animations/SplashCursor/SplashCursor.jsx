...
// Fluid simulation cursor for desktop
function SplashCursor() {
  const canvasRef = useRef(null)
  const [initialized, setInitialized] = useState(false)

  const initFluidSimulation = useCallback((canvas) => {
    ...
    function initFramebuffers() {
      // Safe check for gl context
      if (!gl || !gl.drawingBufferWidth || !gl.drawingBufferHeight) {
        console.error("WebGL context not properly initialized")
        return
      }
    ...
    function getResolution(resolution) {
      // Safe check for gl context
      if (!gl || !gl.drawingBufferWidth || !gl.drawingBufferHeight) {
        return { width: 100, height: 100 } // Default fallback values
      }
  ...
  useEffect(() => {
    ...
    // Wait for the canvas to be properly mounted in the DOM
    setTimeout(() => {
      try {
...
// Main responsive cursor component
export default function ResponsiveCursor() {
  ...
  return (
    <>
      ...
      {/* Only show splash cursor on desktop */}
